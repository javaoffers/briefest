 ...
 * com.javaoffers.thrid.sqlparse.expression for an analytical aggregate like sum(col) or the "all collumns" wildcard like
 * count(*).
 *
 * @author tw
 */
    ...
    public AnalyticExpression(Function function) {
        ...
        if (list != null) {
            if (list.getExpressions().size() > 3) {
                throw new IllegalArgumentException("function object not valid to initialize analytic com.javaoffers.thrid.sqlparse.expression");
            }
