...
### 字段脱敏
<p>
我们通过注解的方式使Model类的字段数据进行脱敏. 脱敏的字段必须声名为String类型. 提供了常用的注解和通用的注解
</p>

- @PhoneNumBlur 手机号脱敏
- @EmailBlur 邮件脱敏. @部分不会进行脱敏
- @IdCardBlur 身份证号脱敏.
- @StringBlur 字符串脱敏. 上面的注解也可以用此注解进行替换.

<p>
@StringBlur 可以用于任何String类型的属性. 不同点是在进行脱敏时不会进行保留特征处理.
比如@EmailBlur 会保留@后部分不进行脱敏处理. 但是@StringBlur则统一按照字符进行脱敏.
</p>

#### 脱敏的特点

<p>
我们可以指定脱敏的开始位置, 从左边或者右边开始脱敏. 默热时中间开始脱敏.
也可以指定脱敏的范围比(0-1), 还可以指定脱敏的符号. 默认是 '*'  
</p>

- percent: 脱敏的范围, 用百分比表示. (0-1)
- blur : 脱敏开始位置. (默认中间)
- blurTag: 脱敏符号. (默认 '*')
...
### 接口default方法中编写jql  
<p>
为了统一jql的编码风格。并且方便项目的维护。防止sql/jql出现的到处都是。因此在设计jql的时候支持了在接口中编写defalue
